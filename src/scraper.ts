...
async function scrapeProduct(page: Page, sku: string, source: string): Promise<Product | null> {
  ...
  try {
    if (source === 'Amazon') {
      ...
      await page.waitForSelector('span.a-price', { timeout: 60000 });  // Ensure we wait for price to load
      await page.waitForSelector('#acrCustomerReviewText', { timeout: 60000 });
      await page.waitForSelector('span.a-icon-alt', { timeout: 60000 });
      ...
      // Scraping the price
      price = await page.$eval('span.a-price', el => el.textContent?.trim() || 'N/A');
      if (price === 'N/A') {
        price = await page.$eval('span.a-price-symbol', el => el.textContent?.trim() || 'N/A');
      }
      ...
      console.log(`Waiting for Walmart product page for SKU: ${sku}`);
      await page.goto(`https://www.walmart.com/product/${sku}`, { waitUntil: 'domcontentloaded' });

      // Use wait for a more stable element, like the body tag to ensure the page has loaded fully
      await page.waitForFunction('body', { timeout: 90000 });  // Ensure page has loaded completely
      await page.waitForFunction('prod-ProductTitle', { timeout: 90000 });
      await page.waitForFunction('span.price-group', { timeout: 90000 });
      await page.waitForFunction('span.review-ratings', { timeout: 90000 });

      // Check for CAPTCHA
      if (await isCaptcha(page)) {
        console.error(`CAPTCHA detected on Walmart for SKU ${sku}`);
        fs.appendFileSync(errorLogFilePath, `CAPTCHA detected on Walmart for SKU ${sku}\n`);
        return null;
      }

      // Scraping title and description
      title = await page.$eval('prod-ProductTitle', el => el.textContent?.trim() || '');
      description = await page.$eval('product-description', el => el.textContent?.trim() || '');

      // Scraping the price
      price = await page.$eval('span.price-group', el => el.textContent?.trim() || 'N/A');
      if (price === 'N/A') {
        price = await page.$eval('span.prod-PriceHero', el => el.textContent?.trim() || 'N/A');
      }

      reviews = await page.$eval('span.review-ratings', el => el.textContent?.trim() || '0 reviews');
